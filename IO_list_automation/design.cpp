...
void Project_put_data_switch(int iCol, int index, wstring cell_text)
{
	switch (iCol)
	{
...
void Project_put_data_listview()
{
	...
	{
		if (Project_valid_row_check(index) == 1)
		{
			...
			for (iCol = 0; iCol <= project.number_collums; iCol++)
			{
				cell_text = Project_get_data_switch(iCol, index);

				String^ textas = wstring_to_system_string(cell_text);

				// Insert items into the list.
				GlobalForm::forma->Design_grid->Rows[grid_cell]->Cells[iCol]->Value = textas;
			}
			grid_cell++;
		}
	...
	if (GlobalForm::forma->Design_grid->RowCount > 0)
	{
		...
		for (int index = 0; index <= project.valid_entries; index++)
		{
			...
			for (iCol = 0; iCol <= project.number_collums; iCol++)
			{
				...
				Project_put_data_switch(iCol, index, cell_text);
			}
			set_progress_value(index);
		}
		Hide_progress();
		...
		if (unstable_release == 1 && project.valid_entries>1)
		{
			Project_save_data(true, " ");
		}
		Project_get_width_list();
	}
	else
	{
		project = {};
...
void Project_data_extract_useful_data()
{
	...
	{
		project.number[row] = int_to_wstring(row, max_digits);
	}
